...
export const variantSchema = z.object({
  ...
  buyingPrice: z
    .preprocess((val) => (typeof val === "string" ? parseFloat(val) : val),
...
export const CreateProductSchema = z.object({
  ...
  variants: z
    .array(variantSchema)
    .min(0, "Variants array is required."),
}).refine(
  (data) => {
    const hasValue = data.weightAmount != null;
    const hasUnit = !!data.weightUnit;
    return !hasValue || hasUnit;
  },
  {
    message: "When specifying a weight amount, you must also specify a unit.",
    path: ["weightAmount"],
  },
);
