  ...
  return (
    <Card className="px-5">
      <CardContent className="space-y-4 px-0">
        <h2 className="text-lg md:text-xl font-semibold text-foreground">Image <span className="text-[#FF3333]">*</span></h2>
        {categoryImage || existingImageUrl ? (
          ...
          <div className="bg-[#616FF514]">
            <Label htmlFor="category-image" className="cursor-pointer">
              <div className="rounded-lg w-full aspect-[4/3] flex flex-col space-y-3 items-center justify-center bg-primary/5 hover:bg-primary/10 transition-colors border border-primary">
                <IconPhoto />
                <div className="flex flex-row gap-3 items-center">
                  <IconUpload className="w-5 h-5 text-primary" />
                  <p className="text-xs md:text-sm text-primary font-medium">Upload Image</p>
                </div>
            ...
            <Input id="category-image" type="file" accept="image/*" onChange={onImageChange} className="hidden" />
          </div>
        )}
      </CardContent>
    </Card>
